...
export function CipherEncoder() {
  ...
  return (
    <div className="glass border border-[#1A1A1A] p-7 font-mono">
      ...
      {mode === 'caesar' && (
        <div className="mb-5">
          <label className="text-[#555] font-mono text-[10px] tracking-widest uppercase block mb-2">SHIFT: <span className="text-[#00FF41]">{shift}</span></label>
      ...
      {activeTab === 'encode' ? (
        <>
          <div className="mb-4">
            <label className="text-[#555] font-mono text-[10px] tracking-widest uppercase block mb-2">INPUT</label>
            <input value={input} onChange={(e) => setInput(e.target.value)} className="tool-input" />
          </div>

          <div className="flex gap-2 mb-4">
            <button onClick={() => setInput(encode())} className="btn-neon flex-1 text-center">ENCRYPT →</button>
            <button onClick={() => setInput(decode())} className="flex-1 text-center" style={{ background: 'rgba(255,183,0,0.1)', border: '1px solid rgba(255,183,0,0.3)', color: '#FFB700', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.8125rem', fontWeight: 700, letterSpacing: '0.05em', padding: '0.625rem 1.5rem', cursor: 'pointer', transition: 'all 0.15s ease' }}>DECRYPT ←</button>
          </div>
